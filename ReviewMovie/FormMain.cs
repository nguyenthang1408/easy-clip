...
namespace ReviewMovie
{
    public partial class FormMain : Form, ILocalizable
    {
        ...
        {
            string confirmMessage = LanguageManager.CurrentLanguage == LanguageManager.Language.En
                ? $"Are you sure you want to open this project?\r\nProject : {projectPath}"
                : $"Bạn có chắc muốn mở Project này?\r\nProject : {projectPath}";
            return MsgBox.Show(confirmMessage, LanguageManager.Get(LangKeys.Common_Notice), MessageBoxButtons.YesNo);
        }
